...
const PROJECT = {
  ...
  links: {
    ...
    repo: '',   // ← add the GitHub repo URL once the project is public (shows a "View code" button)
  ...
  return (
    ...
    >
      <svg width="120" height="300" viewBox="0 0 120 300" fill="none" style={{ overflow: 'visible' }}>
        {/* Fins */}
        <path d="M40 175 L18 225 L40 215 Z" fill="#2563eb" />
        <path d="M80 175 L102 225 L80 215 Z" fill="#2563eb" />
        {/* Body */}
        ...
        {/* Nose tip */}
        <path d="M60 30 Q52 42 53 60 L60 60 Z" fill="#3B82F6" />
        {/* Window */}
        <circle cx="60" cy="100" r="13" fill="#0b1220" stroke="#3B82F6" strokeWidth="3" />
        <circle cx="56" cy="96" r="4" fill="rgba(59,130,246,0.6)" />
        {/* Body band */}
        <rect x="40" y="150" width="40" height="6" fill="#3B82F6" opacity="0.8" />
        {/* Nozzle */}
...
function TelemetryPanel({ active }) {
  ...
  return (
    ...
    >
      ...
      {rows.map((r) => (
        <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '0.4rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
          <span style={{ fontSize: '0.7rem', letterSpacing: '0.1em', color: 'var(--text-dim)' }}>{r.label}</span>
          <span style={{ fontSize: '1rem', fontWeight: 600, color: '#fff', fontVariantNumeric: 'tabular-nums' }}>{r.value}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <section
      ...
      style={{
        ...
        background: 'linear-gradient(180deg, #080808 0%, #0a0f1a 45%, #080808 100%)',
        overflow: 'hidden',
    ...
    >
      ...
      <div style={{ maxWidth: '1150px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        >
          <span style={{ fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.2em', color: 'var(--accent)', textTransform: 'uppercase' }}>
            Featured Project
          </span>
          <span style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 'clamp(2rem, 5vw, 5rem)', alignItems: 'center' }} className="rocket-grid">
          ...
          <div>
            ...
            >
              ...
              <span style={{ display: 'block', fontSize: 'clamp(1rem, 2.2vw, 1.4rem)', fontWeight: 400, color: 'var(--text-muted)', marginTop: '1rem', letterSpacing: 0, lineHeight: 1.5 }}>
                {PROJECT.tagline}
              </span>
            </motion.h2>
